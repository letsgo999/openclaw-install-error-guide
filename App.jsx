...
const App = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans text-slate-900 overflow-hidden">
      {/* 좌측 사이드바 패널 */}
      <nav className="w-80 bg-slate-900 flex flex-col shadow-2xl z-10">
          ...
          {steps.map((step, idx) => (
            ...
            >
              ...
              <div>
                ...
                <p className="font-bold text-sm leading-tight">{step.title.split('. ')[1]}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <main className="flex-1 overflow-y-auto bg-white relative">
        <div className="max-w-4xl mx-auto p-12">
          <header className="mb-12">
            ...
            <h2 className="text-4xl font-black text-slate-900 mb-4">{current.title}</h2>
            <p className="text-xl text-slate-500 leading-relaxed">{current.content.summary}</p>
          </header>
          ...
          <div className="space-y-12">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          <p className="text-sm text-slate-500 leading-relaxed"><span className="font-bold text-red-400">원인:</span> {item.cause}</p>
                          <p className="text-sm text-emerald-600 font-bold flex items-center gap-2">
                            <CheckCircle2 className="w-4 h-4" /> 해결: {item.solution}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <section className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl">
              <h3 className="flex items-center gap-2 text-lg font-bold mb-6 text-blue-400">
                <Code2 className="w-5 h-5" /> 핵심 복습 명령어
              </h3>
              <div className="space-y-3 font-mono">
                {current.content.commands.map((cmd, idx) => (
                  <div key={idx} className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 flex items-center gap-4 group">
                    <span className="text-slate-600 text-xs">$</span>
                    <span className="text-blue-300 group-hover:text-white transition-colors">{cmd}</span>
                  </div>
                ))}
              </div>
            </section>
